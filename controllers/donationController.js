  ...
  try {
    ...
    for (const ngo of verifiedNgos) {
      try {
        ...
        console.error(
          `Failed to send donation email to NGO ${ngo.email}:`,
          mailErr,
        );
      }
    }
...
export const getAllDonations = async (req, res, next) => {
  try {
    ...
    const donations = await Donation.find(filter)
      .populate(
        "donorId",
        "name email phone donorType location address mapLink",
      )
  ...
  try {
    ...
    if (status === "accepted") {
      if (role !== "ngo") {
        return res
          .status(403)
          .json({ success: false, message: "Only NGOs can accept donations" });
      }
        ...
        if (process.env.OAUTH_REFRESH_TOKEN || process.env.MAIL_PASS) {
          try {
            await sendEmail({
              ...
              message: `The NGO ${user.name} is ready to collect the food donation "${donation.foodName}". Please share this 6-digit OTP code with them to verify collection: ${otpCode}`,
              html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
                      ...
                      <p style="font-size: 16px; color: #475569;">The NGO <strong>${user.name}</strong> has accepted your food donation <strong>"${donation.foodName}"</strong>.</p>
                      <p style="font-size: 16px; color: #475569;">Please share this 6-digit one-time password (OTP) with them when they collect the food:</p>
            ...
            console.error(
              "Failed to send collection OTP email on donation acceptance:",
              emailErr,
            );
          }
        } else {
      ...
      if (role !== "ngo") {
        return res
          .status(403)
          .json({ success: false, message: "Only NGOs can mark as collected" });
      }
      ...
      if (current !== "accepted") {
        return res
          ...
          .json({
            success: false,
            message: "Only accepted donations can be collected",
          });
      }
      if (!donation.acceptedBy || donation.acceptedBy.toString() !== userId) {
        return res
          ...
          .json({
            success: false,
            message: "Only the NGO who accepted can collect",
          });
      }
      ...
      if (!otp) {
        return res
          ...
          .json({
            success: false,
            message: "Verification OTP is required to mark as collected",
          });
      }
      if (donation.collectOtp !== otp) {
        return res
          ...
          .json({
            success: false,
            message:
              "Invalid or expired collection OTP. Please ask the donor for the correct code.",
          });
      }
      ...
      if (role !== "ngo") {
        return res
          ...
          .json({
            success: false,
            message: "Only NGOs can complete donations",
          });
      }
      if (current !== "collected") {
        return res
          ...
          .json({
            success: false,
            message: "Only collected donations can be completed",
          });
      }
      const donor = await User.findById(donation.donorId);
      if (donor) {
...
export const deleteDonation = async (req, res, next) => {
  try {
    ...
    if (
      req.user.userId !== donation.donorId.toString() &&
      req.user.role !== "admin"
    ) {
    ...
    if (donor) {
      ...
      console.log(
        `Deducting ${pointsToSubtract} points from donor ${donor.email}. Status was ${donation.status}`,
      );
  ...
  try {
    ...
    if (donation.status !== "completed") {
      return res
        ...
        .json({
          success: false,
          message: "Only completed donations can have impact details",
        });
    }
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message:
            "Only the NGO who accepted this donation can submit impact details",
        });
    }
    ...
    return res
      ...
      .json({
        success: true,
        donation: populated,
        message: "Impact details added successfully",
      });
  } catch (err) {
    next(err);
  }
...
export const sendCollectOtp = async (req, res, next) => {
  try {
    ...
    if (
      role !== "ngo" ||
      !donation.acceptedBy ||
      donation.acceptedBy.toString() !== userId
    ) {
      return res
        .status(403)
        .json({
          success: false,
          message:
            "Only the NGO who accepted this donation can request a collection OTP.",
        });
    }
    ...
    if (donation.status !== "accepted") {
      return res
        ...
        .json({
          success: false,
          message: "OTP can only be sent for accepted donations.",
        });
    }
    ...
    if (process.env.OAUTH_REFRESH_TOKEN || process.env.MAIL_PASS) {
      try {
        ...
        return res
          ...
          .json({
            success: false,
            message: "Failed to send OTP email to donor.",
          });
      }
    } else {
      console.log(
    ...
    return res
      ...
      .json({
        success: true,
        message: "Verification OTP sent to donor email!",
      });
  } catch (err) {
    next(err);
  }
