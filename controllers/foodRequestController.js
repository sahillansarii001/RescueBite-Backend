...
const acceptRequest = async (req, res, next) => {
  try {
    ...
    if (!prepTime) {
      return res
        .status(400)
        .json({ success: false, message: "Preparation time is required" });
    }
    ...
    if (request.status !== "active") {
      return res
        .status(400)
        .json({ success: false, message: "Request is no longer active" });
    }
    ...
    if (donor && ngo) {
      ...
      if (process.env.OAUTH_REFRESH_TOKEN || process.env.MAIL_PASS) {
        try {
          ...
          console.error(
            "Failed to send request collection OTP email on acceptance:",
            emailErr,
          );
        }
      } else {
    ...
    const populated = await FoodRequest.findById(request._id)
      ...
      .populate(
        "fulfilledBy",
        "name email phone location address mapLink donorType",
      );
  ...
  try {
    ...
    if (request.fulfilledBy.toString() !== req.user.userId) {
      return res
        ...
        .json({
          success: false,
          message:
            "Only the donor who fulfilled this request can mark it as prepared.",
        });
    }
    if (request.status !== "accepted") {
      return res
        ...
        .json({
          success: false,
          message: "Only accepted requests can be marked as prepared.",
        });
    }
    ...
    if (ngo && donor) {
      ...
      if (process.env.OAUTH_REFRESH_TOKEN || process.env.MAIL_PASS) {
        try {
          ...
          console.error(
            "Failed to send food prepared notification email to NGO:",
            emailErr,
          );
        }
      } else {
    ...
    if (role !== "ngo" || request.ngoId.toString() !== userId) {
      return res
        ...
        .json({
          success: false,
          message:
            "Only the NGO who created this request can request a collection OTP.",
        });
    }
    ...
    if (request.status !== "accepted" && request.status !== "prepared") {
      return res
        ...
        .json({
          success: false,
          message:
            "OTP can only be requested for accepted or prepared requests.",
        });
    }
  ...
  try {
    ...
    if (role !== "ngo" || request.ngoId.toString() !== userId) {
      return res
        ...
        .json({
          success: false,
          message:
            "Only the NGO who created this request can verify collection.",
        });
    }
    ...
    if (request.status !== "accepted" && request.status !== "prepared") {
      return res
        ...
        .json({
          success: false,
          message: "Only accepted or prepared requests can be collected",
        });
    }
    ...
    if (request.collectOtp !== otp) {
      return res
        ...
        .json({
          success: false,
          message: "Invalid or expired collection OTP code.",
        });
    }
  ...
  try {
    ...
    if (role !== "ngo" || request.ngoId.toString() !== userId) {
      return res
        ...
        .json({
          success: false,
          message: "Only the NGO who created this request can complete it.",
        });
    }
    ...
    if (request.status !== "collected") {
      return res
        ...
        .json({
          success: false,
          message: "Only collected requests can be completed",
        });
    }
...
const deleteRequest = async (req, res, next) => {
  try {
    ...
    if (
      req.user.role !== "admin" &&
      request.ngoId.toString() !== req.user.userId
    ) {
