...
const sendEmail = async (options) => {
  ...
  const senderEmail = process.env.MAIL_USER || 'sahilansari9967747153@gmail.com';
  ...
  if (refreshToken && clientId && clientSecret) {
    ...
    try {
      ...
      console.log(`[sendEmail] Email sent successfully via Gmail API. ID: ${sendData.id}`);
      return;
    } catch (err) {
      console.error(`[sendEmail] Gmail API (OAuth2) FAILED. Falling back to SMTP if available.`);
  ...
  try {
    ...
    console.log(`[sendEmail] Email sent successfully via SMTP. MessageId: ${info.messageId}`);
  } catch (err) {
