...
if (!dbUri) {
  console.error(
    "[Error] MONGODB_URI is not defined in your .env file! Cannot connect to MongoDB Atlas.",
  );
  process.exit(1);
}

// Extract host securely to log verification without printing passwords
const secureHost = dbUri.includes("@")
  ? dbUri.split("@")[1].split("?")[0].split("/")[0]
  : "localhost";
  ...
  .then(async () => {
    ...
    for (const user of users) {
      ...
      if (user.donationCount !== actualCount || user.points !== actualPoints) {
        console.log(
          `User ${user.email}: updating count from ${user.donationCount} to ${actualCount}, points from ${user.points} to ${actualPoints}`,
        );
